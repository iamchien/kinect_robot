...
void pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr & ros_pc2_in)
{
  ...
  point_cloud_t::Ptr pcl_pc_in (new point_cloud_t());
  point_cloud_t::Ptr remainPoints (new point_cloud_t());
  pcl::fromPCLPointCloud2(pcl_pc, *pcl_pc_in);
  ...
  {
    point_cloud_t::Ptr cluster(new point_cloud_t());
...
int main(int argc, char ** argv)
{
  ...
  cluster_array_pub_ = private_nh.advertise < kinect_robot::SegmentedClustersArray > ("clusters", 100);
